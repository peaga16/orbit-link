    ...
    <div className="flex h-full flex-col">
      <div className="flex h-20 items-center justify-between border-b border-white/10 px-5">
        <Link href="/" className="flex items-center gap-3">
          ...
          <div><div className="text-base font-black tracking-[0.17em] text-white">ORBIT</div><div className="text-[8px] font-semibold uppercase tracking-[0.25em] text-cyan-400">Cliente</div></div>
        </Link>
      ...
      <div className="flex-1 px-3 py-5">
        ...
        <nav className="space-y-1.5">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} onClick={() => setMobileOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition ${active ? 'bg-cyan-600 text-white' : 'text-white/55 hover:bg-white/[0.055] hover:text-white'}`}>
      ...
      <div className="border-t border-white/10 p-3">
        ...
        <button onClick={logout} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium text-white/45 transition hover:bg-cyan-500/10 hover:text-cyan-300"><LogOut size={18} /> Sair do painel</button>
      </div>
    </div>
  );
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200 bg-white/85 px-4 backdrop-blur-xl sm:px-7">
          <div className="flex items-center gap-3">
            ...
            <div><div className="text-xs font-semibold uppercase tracking-[0.18em] text-cyan-600">Painel do cliente</div><h1 className="mt-1 text-xl font-bold tracking-tight text-slate-950">{title}</h1></div>
          </div>
