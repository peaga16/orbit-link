...
function OrbitLogo({ compact = false }: { compact?: boolean }) {
  return (
    <div className="flex items-center gap-3">
      <div className="relative flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl border border-cyan-300/30 bg-gradient-to-br from-cyan-400 via-cyan-600 to-violet-700 shadow-[0_0_34px_rgba(34,211,238,.28)]">
      ...
      {!compact && (
        <div className="leading-none">
          ...
          <div className="mt-1 text-[9px] font-semibold uppercase tracking-[0.28em] text-cyan-300">Links profissionais</div>
        </div>
      )}
    </div>
  );
...
function PagePreview({ client }: { client?: LandingClient }) {
  const primary = client?.primaryColor || '#22D3EE';
  const secondary = client?.secondaryColor || '#7C3AED';
  const background = client?.backgroundColor || '#050608';
  ...
  return (
    <div className="relative mx-auto w-full max-w-[390px] rounded-[36px] border border-cyan-200/10 bg-[#0b0d12] p-3 shadow-[0_40px_120px_rgba(0,0,0,.78),0_0_65px_rgba(34,211,238,.08)]">
      <div className="relative min-h-[620px] overflow-hidden rounded-[27px]" style={{ backgroundColor: background }}>
        ...
        <div className="relative p-5">
          ...
          />
          <div className="mx-auto -mt-11 flex h-[88px] w-[88px] items-center justify-center overflow-hidden rounded-3xl border-4 border-[#0b0d12] text-2xl font-black text-white shadow-xl" style={{ backgroundColor: primary }}>
...
function MetricsPanel({ clients }: { clients: LandingClient[] }) {
  ...
  return (
    <div className="relative mx-auto mt-16 max-w-6xl px-4 sm:px-6">
      ...
      <div className="overflow-hidden rounded-[28px] border border-cyan-200/10 bg-[#090b0f] shadow-[0_40px_120px_rgba(0,0,0,.7)]">
          ...
          <aside className="col-span-2 hidden border-r border-white/10 bg-black/20 p-5 md:block">
            ...
            <div className="mt-10 space-y-3">
              {['Visão geral', 'Editar página', 'Desempenho'].map((item, index) => (
                <div key={item} className={`rounded-lg px-3 py-2.5 text-xs font-medium ${index === 0 ? 'bg-cyan-500 text-slate-950' : 'text-white/45'}`}>{item}</div>
              ))}
            </div>
          </aside>
          <div className="col-span-12 p-5 sm:p-7 md:col-span-10">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.25em] text-cyan-300">Painel do cliente</p>
              ...
              {[
                ...
                return (
                  <div key={String(label)} className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
                    <div className="flex items-center justify-between"><div className="text-xs text-white/45">{String(label)}</div><StatIcon size={16} className="text-cyan-300" /></div>
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#050608] text-white">
      <section className="relative min-h-screen border-b border-white/10">
        ...
        <div className="pointer-events-none absolute inset-x-0 top-0 h-[760px] bg-[radial-gradient(circle_at_50%_0%,rgba(34,211,238,.20),transparent_56%)]" />
        <div className="pointer-events-none absolute -left-40 top-32 h-[520px] w-[520px] rounded-full bg-violet-700/15 blur-[150px]" />
        <div className="pointer-events-none absolute right-[-180px] top-16 h-[520px] w-[520px] rounded-full bg-cyan-400/10 blur-[145px]" />
        ...
        <div className="relative z-20 mx-auto max-w-6xl px-4 pt-5 sm:px-6">
          <nav className="flex h-16 items-center justify-between rounded-2xl border border-cyan-200/10 bg-black/75 px-4 shadow-2xl backdrop-blur-xl sm:px-6">
            ...
            <div className="hidden items-center gap-7 text-sm font-medium text-white/65 md:flex">
              ...
              <Link href="/clientes" className="transition hover:text-cyan-300">Páginas</Link>
              <a href="#processo" className="transition hover:text-cyan-300">Como funciona</a>
              <a href="#plano" className="transition hover:text-cyan-300">Plano</a>
              <a href="#contato" className="transition hover:text-cyan-300">Contato</a>
            </div>
          ...
          {menuOpen && (
            <div className="mt-2 rounded-2xl border border-cyan-200/10 bg-black/95 p-4 backdrop-blur-xl md:hidden">
              <div className="grid gap-1 text-sm">
                ...
                <Link href="/cliente/login" className="mt-2 rounded-xl bg-cyan-500 px-4 py-3 text-center font-semibold text-slate-950">Área do cliente</Link>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="relative mx-auto grid max-w-6xl items-center gap-14 px-4 pb-10 pt-20 sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:pt-24">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1.5 text-xs font-semibold text-cyan-200"><span className="h-1.5 w-1.5 rounded-full bg-cyan-300 shadow-[0_0_12px_rgba(34,211,238,.95)]" /> Sua página pronta para usar</div>
            <h1 className="mt-7 text-balance text-5xl font-semibold leading-[.98] tracking-[-0.055em] sm:text-6xl lg:text-7xl">Uma página profissional para reunir <span className="bg-gradient-to-r from-cyan-300 via-cyan-400 to-violet-400 bg-clip-text text-transparent">tudo da sua marca.</span></h1>
            ...
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-3 text-xs text-white/40">{['Página criada para você', 'Editor simples', 'Métricas reais'].map((item) => <span key={item} className="inline-flex items-center gap-2"><Check size={14} className="text-cyan-300" /> {item}</span>)}</div>
          </div>
      ...
      <section id="beneficios" className="relative border-b border-white/10 px-4 py-24 sm:px-6">
        ...
        <div className="relative mx-auto max-w-6xl">
          <div className="grid gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-end"><div><div className="text-xs font-bold uppercase tracking-[0.28em] text-cyan-300">Feito para sua marca</div><h2 className="mt-4 max-w-xl text-balance text-4xl font-semibold tracking-[-0.035em] sm:text-5xl">Você cuida do conteúdo. A estrutura já está pronta.</h2></div><p className="max-w-xl text-base leading-7 text-white/50 lg:justify-self-end">Sem precisar contratar um site novo para cada atualização. A página nasce personalizada e continua fácil de manter pelo painel do cliente.</p></div>
          <div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-4">{features.map((feature, index) => <div key={feature.title} className="group rounded-2xl border border-white/10 bg-white/[0.025] p-6 transition hover:-translate-y-1 hover:border-cyan-300/30 hover:bg-cyan-300/[0.035]"><div className="flex items-center justify-between"><div className="flex h-11 w-11 items-center justify-center rounded-xl border border-cyan-300/20 bg-cyan-300/10 text-cyan-300"><feature.icon size={21} /></div><span className="text-xs font-semibold text-white/20">0{index + 1}</span></div><h3 className="mt-6 text-lg font-semibold">{feature.title}</h3><p className="mt-3 text-sm leading-6 text-white/45">{feature.description}</p></div>)}</div>
        </div>
      </section>

      <section className="px-4 py-24 sm:px-6">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between"><div><div className="text-xs font-bold uppercase tracking-[0.28em] text-cyan-300">Páginas publicadas</div><h2 className="mt-4 text-balance text-4xl font-semibold tracking-[-0.035em] sm:text-5xl">Conheça quem já está na Orbit.</h2><p className="mt-5 max-w-2xl text-white/50">Todo novo cliente cadastrado aparece automaticamente na vitrine pública.</p></div><Link href="/clientes" className="orbit-btn-secondary shrink-0">Ver todos os clientes <ArrowRight size={17} /></Link></div>
          <div className="mt-14 grid gap-6 lg:grid-cols-2">
            {clients.slice(0, 4).map((client) => (
              <article key={client.id} className="group overflow-hidden rounded-[26px] border border-white/10 bg-[#090b0f] transition hover:border-cyan-200/20">
                <div className="relative min-h-[370px] overflow-hidden p-6 sm:p-8" style={{ background: client.backgroundImage ? `linear-gradient(rgba(0,0,0,.62),rgba(0,0,0,.78)), url(${client.backgroundImage}) center/cover` : `linear-gradient(145deg, ${client.backgroundColor}, #050608)` }}>
                  ...
                  <div className="relative z-10 mx-auto max-w-sm rounded-[28px] border border-white/10 bg-black/45 p-5 shadow-2xl backdrop-blur-xl transition duration-500 group-hover:-translate-y-2">
                    ...
                    <div className="mx-auto -mt-8 flex h-16 w-16 items-center justify-center overflow-hidden rounded-2xl border-4 border-[#0b0d12] text-xl font-black text-white" style={{ backgroundColor: client.primaryColor }}>{client.logo ? <img src={client.logo} alt="" className="h-full w-full object-cover" /> : client.name.charAt(0)}</div>
      ...
      <section id="processo" className="border-y border-white/10 bg-[#07090d] px-4 py-24 sm:px-6">
        <div className="mx-auto max-w-6xl"><div className="grid gap-12 lg:grid-cols-[.85fr_1.15fr] lg:items-center"><div><div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1.5 text-xs font-semibold text-cyan-200"><ShieldCheck size={14} /> Serviço completo</div><h2 className="mt-5 text-balance text-4xl font-semibold tracking-[-0.035em] sm:text-5xl">Você não precisa montar o sistema. Você recebe a página pronta.</h2><p className="mt-5 max-w-lg leading-7 text-white/50">A criação fica com a Orbit. O painel serve para manter a página sempre atualizada, sem depender de alterações técnicas.</p></div><div className="space-y-3">{[[Palette, 'Definimos o visual', 'Organizamos identidade, cores, imagens e estrutura de acordo com sua marca.'], [Globe2, 'Criamos e publicamos', 'Sua página recebe um endereço público e já chega pronta para compartilhar.'], [Pencil, 'Você mantém o conteúdo', 'Entre no painel para trocar links, imagens, fundo, textos, Pix e acompanhar métricas.']].map(([Icon, title, description], index) => { const StepIcon = Icon as typeof Palette; return <div key={String(title)} className="flex gap-5 rounded-2xl border border-white/10 bg-white/[0.025] p-5 sm:p-6"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-400 to-violet-600 text-slate-950 shadow-[0_0_30px_rgba(34,211,238,.18)]"><StepIcon size={21} /></div><div><div className="text-xs font-semibold uppercase tracking-[0.2em] text-white/25">Etapa 0{index + 1}</div><h3 className="mt-2 text-lg font-semibold">{String(title)}</h3><p className="mt-2 text-sm leading-6 text-white/45">{String(description)}</p></div></div>; })}</div></div></div>
      </section>
      ...
      <section id="plano" className="relative overflow-hidden px-4 py-24 sm:px-6">
        ...
        <div className="relative mx-auto max-w-5xl">
          <div className="text-center"><div className="text-xs font-bold uppercase tracking-[0.28em] text-cyan-300">Um único plano</div><h2 className="mt-4 text-balance text-4xl font-semibold tracking-[-0.035em] sm:text-5xl">Tudo o que sua página precisa, sem escolhas complicadas.</h2><p className="mx-auto mt-5 max-w-2xl text-white/50">Por enquanto, a Orbit trabalha com um preço fixo e todos os recursos essenciais inclusos.</p></div>
          <div className="relative mx-auto mt-12 max-w-3xl overflow-hidden rounded-[30px] border border-cyan-300/20 bg-gradient-to-br from-cyan-300/[0.09] via-white/[0.035] to-violet-500/[0.08] p-1 shadow-glow">
            <div className="rounded-[26px] border border-white/10 bg-[#080a0e]/95 p-7 backdrop-blur-xl sm:p-10">
              <div className="grid gap-9 lg:grid-cols-[.85fr_1.15fr] lg:items-center">
                <div>
                  <div className="inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-3 py-1.5 text-xs font-semibold text-cyan-200"><Zap size={14} /> Plano Orbit</div>
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">
                  {planFeatures.map((feature) => <div key={feature} className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/[0.035] px-4 py-3 text-sm text-white/70"><div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-cyan-300/15 text-cyan-300"><Check size={14} /></div>{feature}</div>)}
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="contato" className="relative px-4 py-24 sm:px-6">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_50%,rgba(34,211,238,.15),transparent_55%)]" />
        <div className="relative mx-auto max-w-5xl overflow-hidden rounded-[30px] border border-cyan-300/20 bg-gradient-to-br from-[#06202a] via-[#0a1018] to-[#07070a] p-8 text-center shadow-glow sm:p-14">
          <div className="pointer-events-none absolute -right-24 -top-24 h-64 w-64 rounded-full bg-violet-600/20 blur-[90px]" />
          <div className="relative mx-auto flex h-14 w-14 items-center justify-center rounded-2xl border border-cyan-300/30 bg-cyan-400 text-slate-950 shadow-[0_0_36px_rgba(34,211,238,.25)]"><MousePointerClick size={25} /></div>
